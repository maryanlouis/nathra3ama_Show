...
Data alldata = new Data(categories: [], vedios: [], );
  ...
  await _messageRef.get().then((DocumentSnapshot snapshot) async {
    ...
    //int n = num as int;
         ...
         myvedios e = new myvedios(
             vedio_title: snapshot['title'] as String,
             vedio_id: snapshot['id'] as String,
             vedio_category: snapshot['categoryId'] as int,
             serial: snapshot['serial'] as int,
             vedio_description: snapshot['description'] as String,
         );
         allVedios.add(e);
...
Future<void> findVediosLinks(String num, String x) async {
  ...
  await _messageRef.get().then((DocumentSnapshot snapshot) {
    //snapshot.docs.forEach((document) async {
      ...
      resource_link d = new resource_link(
        ...
        resource_title: snapshot['resourceTitle'] as String,
        vedio_serial: snapshot['vedioSerial'] as int
      );
      //int k = num as int;
  ...
  await _messageRef.get().then((DocumentSnapshot snapshot) {
    //snapshot.docs.forEach((document) async {
      ...
      mycategories d = new mycategories(
        category_id: snapshot["categoryId"] as int,
        category_title: snapshot["categoryTitle"] as String,
      );
      allCategories.add(d);
